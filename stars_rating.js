...
function setRating($el, rating, isUser, starGlyph) {
    ...
    $el.trigger('change');
    var save_data = {rating: rating };
    Meteor.call('insertRating', save_data, function(error, result){
      if(error){
        console.log('error');
      }else{
        console.log('success');
...
UI.registerHelper('showData', function(nextHours) {
  data = [];
  var ret = Rating.find().fetch().map(function(val) {
      data.push(val.rating);
   })
    return data;
